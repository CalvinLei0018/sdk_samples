...
//                              " ! appsink name=appsink1";
    ...
    std::string pipelineStr = "v4l2src device=/dev/video0 ! video/x-raw,width=";
                    ...
                    " ! tee name=t ! queue leaky=2 max-size-bytes=165888000 ! videoconvert ! video/x-raw,format=" + _publishFormat +
                    " ! appsink drop=true max-buffers=10 name=appsink0 t. " +
                    " ! queue leaky=2 max-size-bytes=82944000 ! videoconvert ! video/x-raw,format=RGBA ! appsink drop=true max-buffers=5 name=appsink1";
